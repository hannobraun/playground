...
#[test]
fn addition_wraps() {
    let mut program = Application::compile_and_run("2147483647 1 +");
...
#[test]
fn division() {
    let mut program = Application::compile_and_run("11 4 /");
...
fn division_triggers_division_by_zero() {
    let mut program = Application::compile_and_run("1 0 /");
...
fn division_triggers_integer_overflow() {
    let mut program = Application::compile_and_run("-2147483648 -1 /");
    assert_eq!(program.operands(), &vec![]);
...
fn multiplication_wraps() {
    let mut program = Application::compile_and_run("1073741824 2 *");
    assert_eq!(program.operands(), &vec![-2147483648]);
    assert_eq!(program.effect(), None);
}
